...
const int N_CITY_COUNT=4;  //城市数量
const int N_ANT_COUNT=4; //蚂蚁数量   根据城市数量设定 一般不超过城市距离
const int N_IT_COUNT=10000; //迭代次数


//const int N_CITY_ID[]={0,1,2 ,3 };   //城市ID
int N_CITY_ID[N_CITY_COUNT]={0,1,2 ,3};//城市ID
const int N_START_CITY_ID=0 ;  //出发城市ID
const int N_END_CITY_ID= 1;  //终点城市ID
const int N_MUST_ID[]={2,3};        //必经节点ID

const double DBQ=100.0; //总的信息素
const double DB_MAX=10e9; //一个标志数，10的9次方

double g_Trial[N_CITY_COUNT][N_CITY_COUNT]; //两两节点间信息素，就是环境信息素
...
void CAnt::Init()
{
    ...
    for (int i=0;i<N_CITY_COUNT;i++)
    {
		...
		m_nAllowedCity[N_END_CITY_ID]=0 ;  //设置最终节点已经去过  无法选择
...
int CAnt::ChooseNextCity()
{
    ...
    for (int i=0;i<N_CITY_COUNT;i++)
    {
        ...
        {
            prob[i]=pow(g_Trial[m_nCurCityNo][i],ALPHA)*pow(1.0/g_Distance[m_nCurCityNo][i],BETA); //该城市和当前城市间的信息素   这里可以优化POW函数。
			if (prob[i]<0)
			{
              prob[i]=0.0;
			}
